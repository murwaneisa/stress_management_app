...
class StudentStatistics:
    ...
    def showStats(self, program, degree):
        ...
        q1 = "SELECT stats_weeknr,user_id,stats_stress "
        q2 = "FROM user,stats "
        ...
        values = {"stress": [], "weeknr": [], "user_id": []}
        for r in result:
            values["weeknr"].append(r[0])
            values["user_id"].append(r[1])
            values["stress"].append(r[2])

        students = set(values["user_id"])
        weeks = set(values["weeknr"])
        ...
        for i, v in enumerate(values["weeknr"]):
        ...
        for w in week_dic:
            ...
            values.append({'weeknr': w, 'stress': week_stress,'avg_stress': avg_stress})
        ...
        for a in activities:
            with st.expander(a.upper() + " STATS"):
                ...
                # make query
                q1 = "SELECT stats_weeknr,user_firstname,user_lastname,stats_stress,stats_"+a+" "
                q2 = "FROM user,stats "
                q3 = "WHERE (user.user_id = stats.stats_userid)"
                ...
                for r in result:
                    ...
                    dic = {"weeknr": r[0], "name": r[1]+" "+r[2], "hours": r[4], "stress": r[3], "boundary": b}
                    values.append(dic)
                ...
                chart = alt.Chart(data).mark_circle().encode(
                    x='weeknr:Q',
                    y='hours:Q',
                    tooltip=['name:O', 'weeknr:Q', 'hours:Q', 'stress:Q'],
                    size='stress:Q',
